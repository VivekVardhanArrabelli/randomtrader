...
def run() -> None:
    ...
    log(f"  Max risk per trade: {config.resolved_max_risk_per_trade():.0%}")
    log(f"  Max total exposure: {config.resolved_max_total_exposure():.0%}")
    log(f"  Daily loss limit: {config.resolved_daily_loss_limit():.0%}")
    log(f"  Max open positions: {config.resolved_max_open_positions()}")
